...
testSimpleWithExpr :: String -> String -> ST.Scope -> IO ()
testSimpleWithExpr p t s = do
...
spec = describe "Variable Declarations" $ do
    ...
    it "allows declare variables of type `<n>-miracle`" $
        testSimpleWithExpr "<1>-miracle" ">-miracle" 1
